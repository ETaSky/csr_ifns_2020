...
ggplot(BW_f_3, aes(x = TimePoint_Weeks, y = Weight_g)) +
    geom_point(size = 1.5, aes(color = Flag)) +
    geom_line(aes(group = MouseID, color = Flag)) +
    stat_summary(data = BW_f_3 %>% filter(BirthMode=="VF") %>% select(-BirthMode), geom = "ribbon", fun.data = "mean_sdl", fun.args = list(mult = n_sd), fill = "#377EB8", alpha = 0.3) +
    stat_summary(data = BW_f_3 %>% filter(BirthMode=="VF") %>% select(-BirthMode), geom = "line", fun = "mean", color = "#377EB8", size = 1) +
...
BW_f_3 %>% group_by(Sex, BirthMode, MouseID) %>% filter(TimePoint_Weeks>=6) %>% summarise(N = n(), Pct_over = sum(Flag=="Overweight")/N, Pct_under = sum(Flag=="Underweight")/N) %>% mutate(Over = Pct_over>=0.7, Under = Pct_under>=0.7) %>% group_by(Sex, BirthMode) %>% summarise(N_sub = n(), N_over = sum(Over), N_under = sum(Under)) %>% mutate(OR_over = N_over/(N_sub-N_over)/(N_over[1]/(N_sub[1]-N_over[1])))
```
...
ggplot(BW_f_4, aes(x = TimePoint_Weeks, y = Weight_gain)) +
    geom_point(size = 1.5, aes(color = Flag)) +
    geom_line(aes(group = MouseID, color = Flag)) +
    stat_summary(data = BW_f_4 %>% filter(BirthMode=="VF") %>% select(-BirthMode), geom = "ribbon", fun.data = "mean_sdl", fun.args = list(mult = n_sd), fill = "#377EB8", alpha = 0.3) +
    stat_summary(data = BW_f_4 %>% filter(BirthMode=="VF") %>% select(-BirthMode), geom = "line", fun = "mean", color = "#377EB8", size = 1) +
    #stat_summary(data = BW_f %>% filter(BirthMode=="CS") %>% select(-BirthMode), geom = "ribbon", fun.data = "mean_sdl", fun.args = list(mult = n_sd), fill = "#E41A1C", alpha = 0.3) +
    #stat_summary(data = BW_f %>% filter(BirthMode=="CS") %>% select(-BirthMode), geom = "line", fun.y = "mean", color = "#E41A1C", size = 1) +
    facet_grid(Sex ~ BirthMode) +
    scale_color_manual(values = c("Overweight" = "red", "Normal" = "grey30", "Underweight" = "green")) +
...
BW_f_4 %>% group_by(Sex, BirthMode, MouseID) %>% filter(TimePoint_Weeks>=6) %>% summarise(N = n(), Pct_over = sum(Flag=="Overweight")/N, Pct_under = sum(Flag=="Underweight")/N) %>% mutate(Over = Pct_over>=0.7, Under = Pct_under>=0.7) %>% group_by(Sex, BirthMode) %>% summarise(N_sub = n(), N_over = sum(Over), N_under = sum(Under)) %>% mutate(OR_over = N_over/(N_sub-N_over)/(N_over[1]/(N_sub[1]-N_over[1])))
```
